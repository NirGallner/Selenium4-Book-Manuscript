...
After these changes, `Datepicker` class evolves into the following form, but the body of `pick` method doesn't change from Listing 
ifdef::chapter_id[]
[[{chapter_id}-datepicker-source]]
endif::[]
, what has changed is the types of the instance variables.

ifdef::chapter_id[]
[[{chapter_id}-datepicker-sourcePersonTable]]
endif::[]
[source,java]
